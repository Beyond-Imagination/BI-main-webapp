<template>
  <div class="photo_container" :class="{blurEffect : photoUrl}">
    <PhotoCard v-for="(photo) in photos" :photo="photo" @setPhoto="setPhoto"></PhotoCard>
  </div>
  <div v-if="photoUrl" v-on:mouseover="mouseover" v-on:mouseleave="mouseleave"
    ...
    <div v-show="active"
         style="height : 60px; width:500px; position:absolute; bottom:0 ;left:0; background-color:rgba(0, 0, 0, 0.8);">
      <div>{{ description }}</div>
